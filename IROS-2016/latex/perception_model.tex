...
For the purpose of social navigation, we convert this samples to a set of particles. Even though the set of samples are fixed, the set of particles can vary. This is because each of the samples is a joint sample which represents the whole occupancy grid. When there are multiple people in the environment, each of the sample can be decomposed into a set of particles which will be proportional to the number of people in the environment. %We represent the set of particles by $N_{p}$. 