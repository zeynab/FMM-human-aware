...
In the following, we will go into the details of computing social costs with the particles obtained from the MCMC tracker. Using the concept of layered costmaps similar to \cite{lu2014iros}, an FMM planner uses this information for performing the replanning.% It should be emphasized that the proposed methods are planner-independent and applicable as far as the costmap concept is supported. 
...
The core idea we propose for incorporating uncertainty in the costmap is to compute an \textit{expectation} based costmap. In other words, we treat the probability map of the presence of people in the environment as a two dimensional signal and convolve it with the conventional Gaussian costmap, to obtain the final probabilistic social costs. If $K$ is a 2D Gaussian kernel representing the social costmap, given the $ProbabilityMap$ of the environment which is a discrete map representation containing the probability of each cell being occupied by a person, the social cost of each grid cell is:   
\begin{equation}
...
By convolving all the samples from the MCMC tracker with the social costmap, we compute an \textit{expected costmap} incorporating all the uncertainty in the environment. This is a principled approach to solve the problem 
